...
import { z } from "zod"
import { runCodexCLI, extractAgentMessage } from "./codexRunner.js"
...
// @ts-ignore - SDK uses Zod v4 internally, may conflict with project's Zod v3
server.tool(

  "review-plan",
  "This tool MUST be used in PLAN MODE before any final output.",
  {
  ...
  async (
    ...
    try {
      ...
      logger.info({ agentMessageLength: agentMessage.length }, "Agent message extracted")

      const parsed = CodexReviewResultSchema.safeParse(JSON.parse(agentMessage))
      if (!parsed.success) {
        logger.warn({ error: parsed.error.format() }, "AI response did not match schema")
      ...
      logger.info({ status: result.status, confidence: review.confidence_score }, "review-plan tool completed")
      return {
        content: [
          {
            type: "text",
